...
begin
  ...
  begin
    if 7 < DaysBetween(Date, TUtilities.GetFileCreationTime(LFileList[i])) then
...
begin
  ...
  try
    ...
    begin
      LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x]', [GetCurrentThreadId]) + ' FATAL - ' + AValue[i];
      FLogMessageQueue.Enqueue(LLogMessage);
    end;
    SetEvent(FLogEvent);
  finally
...
begin
  ...
  try
    for i := 0 to (AValue.Count - 1) do
    begin
      LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x]', [GetCurrentThreadId]) + ' ERROR - ' + AValue[i];
      FLogMessageQueue.Enqueue(LLogMessage);
    end;
    SetEvent(FLogEvent);
  finally
...
begin
  ...
  try
    for i := 0 to (AValue.Count - 1) do
    begin
      LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x]', [GetCurrentThreadId]) + ' WARN  - ' + AValue[i];
      FLogMessageQueue.Enqueue(LLogMessage);
    end;
    SetEvent(FLogEvent);
  finally
...
begin
  ...
  try
    for i := 0 to (AValue.Count - 1) do
    begin
      LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x]', [GetCurrentThreadId]) + ' INFO  - ' + AValue[i];
      FLogMessageQueue.Enqueue(LLogMessage);
    end;
    SetEvent(FLogEvent);
  finally
...
begin
  ...
  try
    for i := 0 to (AValue.Count - 1) do
    begin
      LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x]', [GetCurrentThreadId]) + ' DEBUG - ' + AValue[i];
      FLogMessageQueue.Enqueue(LLogMessage);
    end;
    SetEvent(FLogEvent);
  finally
...
begin
  if FLogLevel < LOG_FATAL then
    EXIT;
  //LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x/0x%.8x]', [GetCurrentProcessId, GetCurrentThreadId]) + ' FATAL - ' + AValue;
  LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + String.Format('[0x%.8x]', [GetCurrentThreadId]) + ' FATAL - ' + String.Format(AValue, AArgs);
  Lock;
  try
    FLogMessageQueue.Enqueue(LLogMessage);
    SetEvent(FLogEvent);
...
begin
  if FLogLevel < LOG_ERROR then
    EXIT;
  //LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x/0x%.8x]', [GetCurrentProcessId, GetCurrentThreadId]) + ' ERROR - ' + AValue;
  LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + String.Format('[0x%.8x]', [GetCurrentThreadId]) + ' ERROR - ' + String.Format(AValue, AArgs);
  Lock;
  try
    FLogMessageQueue.Enqueue(LLogMessage);
    SetEvent(FLogEvent);
...
begin
  if FLogLevel < LOG_WARN then
    EXIT;
  //LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x/0x%.8x]', [GetCurrentProcessId, GetCurrentThreadId]) + ' WARN  - ' + AValue;
  LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + String.Format('[0x%.8x]', [GetCurrentThreadId]) + ' WARN  - ' + String.Format(AValue, AArgs);
  Lock;
  try
    FLogMessageQueue.Enqueue(LLogMessage);
    SetEvent(FLogEvent);
...
begin
  if FLogLevel < LOG_INFO then
    EXIT;
  //LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x/0x%.8x]', [GetCurrentProcessId, GetCurrentThreadId]) + ' INFO  - ' + AValue;
  LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + String.Format('[0x%.8x]', [GetCurrentThreadId]) + ' INFO  - ' + String.Format(AValue, AArgs);
  Lock;
  try
    FLogMessageQueue.Enqueue(LLogMessage);
    SetEvent(FLogEvent);
...
begin
  if FLogLevel < LOG_DEBUG then
    EXIT;
  //LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + Format('[0x%.8x/0x%.8x]', [GetCurrentProcessId, GetCurrentThreadId]) + ' DEBUG - ' + AValue;
  LLogMessage := FormatDateTime('YYYY-MMM-DD hh:nn:ss.zzz', Now) + ' ' + String.Format('[0x%.8x]', [GetCurrentThreadId]) + ' DEBUG - ' + String.Format(AValue, AArgs);
  Lock;
  try
    FLogMessageQueue.Enqueue(LLogMessage);
    SetEvent(FLogEvent);
